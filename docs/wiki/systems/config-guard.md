...
Prompt answers are cached by verified process generation (PID, process start time, and executable), access kind, and decision reason rather than policy scope. One process scanning multiple protected roots therefore receives one prompt for equivalent reads; both Allow and Deny answers apply only to that process generation. For a cross-owner helper running under its owning process, an explicit Allow additionally creates a runtime-only ancestry authorization keyed by the exact verified helper executable path and exact verified owning ancestor executable path. That authorization applies across all owned scopes and all later owner process generations and logical Pi sessions using that exact pair until Config Guard restarts. A different helper path or different owner path does not reuse it, even when the executable basenames match. Explicit Deny, default decisions, missing verified executable identity, and Config Guard restart do not create or reuse ancestry authorization. Before the serialized prompt worker presents each queued dialog, it rereads the process start time from procfs. A missing process or changed start time discards that stale dialog and applies the configured default to its pending events, so killing a blocked process cannot leave later prompts queued behind it. If process inspection cannot provide the required executable and process-generation identity, Config Guard logs the identity failure, evaluates policy using the available or explicit `unknown` subject, and does not cache the ordinary prompt answer or create ancestry authorization without both verified executable paths.
