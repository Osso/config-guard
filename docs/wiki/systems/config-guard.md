...
Prompt answers are cached by verified process generation (PID, process start time, and executable), access kind, and decision reason rather than policy scope. One process scanning multiple protected roots therefore receives one prompt for equivalent reads; both Allow and Deny answers apply only to that process generation. For a cross-owner helper running under its owning process, an explicit Allow additionally creates a runtime-only ancestry authorization keyed by the exact helper executable, the root generation of the nearest contiguous chain of matching owner executables, and owned scope. The root generation is the long-lived owner-session root for short-lived detached runners in that chain, so those runners share one authorization. A different owner-session root prompts again; a matching owner executable separated by a non-owner process does not share authorization. The authorization covers all access kinds in the scope until Config Guard restarts. Explicit Deny and default decisions do not create ancestry authorization. Before the serialized prompt worker presents each queued dialog, it rereads the process start time from procfs. A missing process or changed start time discards that stale dialog and applies the configured default to its pending events, so killing a blocked process cannot leave later prompts queued behind it. If process inspection cannot provide the required executable and process-generation identity, Config Guard logs the identity failure, evaluates policy using the available or explicit `unknown` subject, and does not cache the prompt answer or create ancestry authorization.
