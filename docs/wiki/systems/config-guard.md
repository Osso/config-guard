...
The deployed policy also strictly owns `$HOME/.config/curseforge` for the `curseforge` executable with no additional allowed subjects and `deny_non_owner = true`. The credential directory should use Unix mode `0700` and its credential file mode `0600`; those Unix permissions are the primary at-rest boundary, not encryption. Config Guard adds executable-path enforcement. The deployed service is currently guard mode with global `fail_open = true`, but this rule returns `Deny` before prompt or fail-open handling. Sibling paths such as `$HOME/.config/curseforge-helper` remain governed independently.
