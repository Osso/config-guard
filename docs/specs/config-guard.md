...
- [x] Treat Unix directory mode `0700` and credential-file mode `0600` as the primary at-rest boundary for CurseForge credentials; Config Guard adds executable-path enforcement, not encryption. The deployed guard mode has global `fail_open = true`, but this rule remains strict because its denial precedes prompt and fail-open handling.
