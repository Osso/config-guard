...
- [x] On an explicit Allow for a cross-owner helper, select the nearest verified process ancestor whose executable basename is configured as an owner anywhere in policy, then create a runtime-only ancestry authorization keyed by the exact verified helper executable path and exact verified configured-owner ancestor executable path; apply it across all Config Guard protected scopes, including scopes with a different configured owner, and all later process generations and logical Pi sessions using that exact pair until Config Guard restarts.
- [x] Require a new prompt when no matching runtime ancestry authorization exists; a different helper path, a different ancestor executable path even with the same basename, no configured-owner ancestor, an unverified ancestry chain, explicit Deny, default decisions, and Config Guard restart must not reuse it.
...
- [x] Protect `$HOME/.config/curseforge` as an owned directory for `curseforge` with no additional allowed subjects and `deny_non_owner = true`; allow the owner all access kinds, deny listed non-owner processes before prompt or fail-open handling, and leave sibling paths unchanged.
- [x] Treat Unix directory mode `0700` and credential-file mode `0600` as the primary at-rest boundary for CurseForge credentials; Config Guard adds executable-path enforcement, not encryption, and does not set or validate those mode bits. The policy seed has global `fail_open = true`; when deployed in guard mode, this rule remains strict because its denial precedes prompt and fail-open handling, except for the global final-component `.gitignore` allow rule.
